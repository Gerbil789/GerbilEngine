...
namespace Engine
{
	...
	Mesh* MeshImporter::LoadMesh(const std::filesystem::path& path)
	{
			...
			auto readVec3Array = [&](const std::string& attr) -> std::vector<glm::vec3>
				{
					...
					if (!attributes.count(attr)) return result;

					const auto& accessor = model.accessors[attributes.at(attr)];
		...
		// ---- Wireframe indices ----
		wireIndices.reserve(indices.size() * 2);
		...
		MeshSpecification spec{ vertices, indices, wireIndices };
