...
namespace editor
{
	...
	void EditorRenderer::Initialize(const engine::Window& window)
	{
		...
		constexpr const char* imgui_ini = "resources/layouts/imgui.ini";
	...
	void EndFrame()
	{
		...
		wgpu::Surface surface = engine::Application::s_Window.GetSurface();
		...
		{
			std::stringstream ss;
			ss << surfaceTexture.status;

			LOG_ERROR("Failed to get current surface texture. status: {}", ss.str());
			return;
		}
	...
	void EditorRenderer::ResetLayout()
	{
		const std::filesystem::path default_iniPath = "resources/layouts/default.ini";
	...
	void EditorRenderer::SaveLayout()
	{
		const std::filesystem::path iniPath = "resources/layouts/imgui.ini";
