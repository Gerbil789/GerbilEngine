...
namespace Editor
{
	struct SelectionWithDeletion : ImGuiSelectionBasicStorage
	{
		...
		void ApplyDeletionPostLoop(ImGuiMultiSelectIO* ms_io, std::vector<Engine::AssetRecord>& items, int item_curr_idx_to_select)
		{
			// Rewrite item list (delete items) + convert old selection index (before deletion) to new selection index (after selection).
			// If NavId was not part of selection, we will stay on same item.
			std::vector<Engine::AssetRecord> new_items;
